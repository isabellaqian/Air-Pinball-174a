...
export class Assignment3 extends Scene {
  constructor() {
    ...
    this.shapes = {
      ...
      planet3ring: new defs.Torus(40, 20),
    ...
    this.materials = {
      ...
      //        (Requirement 4)
      sun: new Material(new Gouraud_Shader(), {
        ambient: 1,
      ...
      // 3 subdivisions, maximum specular, low diffuse
      planet2gourand: new Material(new Gouraud_Shader(), {
        ambient: 0,
        diffusivity: 0.2,
      ...
      planet3ring: new Material(new defs.Phong_Shader(), {
        ambient: 0,
        color: hex_color("#B08040"),
      }),
  ...
  display(context, program_state) {
    ...
    let rad_sun = 2 + Math.sin(((2 * Math.PI) / 8) * t);
    let trans_sun = model_transform.times(
      Mat4.scale(rad_sun, rad_sun, rad_sun)
    );
    //white -> red color for sun
    let col = (1 + Math.sin(((2 * Math.PI) / 8) * t)) / 2;
    let color_sun = color(1, col, col, 1);

    //stop it for a sec
    // color_sun = color(1, 1, 1, 1);
    // rad_sun = 3;
    // trans_sun = model_transform.times(Mat4.scale(3, 3, 3));

    // TODO: Lighting (Requirement 2)
    const light_position = vec4(0, 0, 0, 1);
    // The parameters of the Light are: position, color, size
    program_state.lights = [
      new Light(light_position, color_sun, 10 ** rad_sun),
    ];
    this.shapes.sun.draw(
      context,
      program_state,
      trans_sun,
      this.materials.sun.override({ color: color_sun })
    );

    //draw planets
    //planet1
    let trans_planets1 = model_transform
    ...
    //planet2
    let trans_planets2 = model_transform
      .times(Mat4.rotation(t * 0.8, 0, 1, 0))
    ...
    if (Math.floor(t % 2) == 0) {
      ...
      this.shapes.planet2.draw(
        ...
        program_state,
        trans_planets2,
        this.materials.planet2gourand
      );
    }

    //planet3
    let trans_planets3 = model_transform
      .times(Mat4.rotation(t * 0.6, 0, 1, 0))
      .times(Mat4.translation(11, 0, 0));
    let trans_ring = trans_planets3.times(Mat4.scale(4, 4, 0.2));
    ...
    //planet4
    let trans_planets4 = model_transform
      .times(Mat4.rotation(t * 0.4, 0, 1, 0))
      .times(Mat4.translation(14, 0, 0));
    let trans_moon = trans_planets4
      .times(Mat4.rotation(t, 0, 1, 0))
    ...
    //set up attached camera
    this.planet_1 = trans_planets1;
    this.planet_2 = trans_planets2;
    this.planet_3 = trans_planets3;
    this.planet_4 = trans_planets4;
    this.moon = trans_moon;
    if (this.attached) {
      if (this.attached() == this.initial_camera_location) {
        ...
        desired = desired.map((x, i) =>
          Vector.from(program_state.camera_transform[i]).mix(x, 0.1)
        );
...
class Ring_Shader extends Shader {
  ...
  fragment_glsl_code() {
    ...
    return (
      ...
      `
        void main(){
            float constant = sin(20.0 * distance(point_position.xyz, center.xyz));
            gl_FragColor = constant * vec4(vec3(176.0/255.0, 128.0/255.0, 64.0/255.0), 1.0);
        }`
    );
  }
}
